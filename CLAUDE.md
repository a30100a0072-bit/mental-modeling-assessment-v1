...
## 🧪 測驗 UX 紀律（2026-05-03 踩過的教訓）

本站定位是**評測站不是星座 app**，UX 必須維持專業感。動測驗流程（assessment.html / questions.js / engine.js / quiz-ux.js）前先讀此節。

### 鐵則：測驗中途不揭露任何分析

使用者尚未走完測驗（result-area 沒顯示）前，**禁止**在 UI 暴露以下任何資訊：

- ❌ 已預測的型（如「已可預測你是 INTJ」）
- ❌ 信心 / 機率 / 置信度（如「信心 73%」）
- ❌ 軸線分析（如「偵測到 T/F 軸接近 50/50」）
- ❌ 認知功能偵測（如「偵測到核心向量 [Ni/Te] 高度重合」）
- ❌「直接看結果」這類 shortcut 結論按鈕（即使技術上算得出）

### 可以做什麼

- ✅ **存檔提示** 包裝：「進度已自動存檔，下次造訪會從這題接續」+「暫停測驗」按鈕
- ✅ **進度資訊** 純題數：「剩 N 題」「已完成 X / Y」「最後 N 題」
- ✅ **後台動態挑題**邏輯保留（如 phase 5 4 軸最模糊→3 題決勝題），但 UI 文案要中性，**不揭露為什麼挑這幾題**
- ✅ **GA / KV / D1 telemetry** 紀錄信心 / 軸線 / threshold 等分析資料（使用者看不到）
- ✅ **結果頁**（result-area 顯示後）什麼都能秀：型 / 信心 / 互補配對 / Beebe 堆疊 / 4 軸 trend

### Phase 5 文案約束

```
header: "PHASE 05: 最終校準"
desc: "最後 N 題，請憑直覺作答以完成測驗。"
```

絕不再回到舊版「階層解耦探針 (高增益)」/「偵測到核心向量重合」/「軸線決勝校準」這類揭露偵測過程的文案。任何想動 phase 5 文案的提議，預設拒絕。

### 純函式保留說明

`engine.js` 內 `evaluateConfidence` / `canStopEarly` / `calculateAxisProbabilities` / `findMostAmbiguousAxis` 是後台分析用純函式（vitest 43 case 測試覆蓋），**不應從測驗 UI 直接呼叫顯示給使用者**。後台 / dashboard / GA pipeline 用沒問題。
