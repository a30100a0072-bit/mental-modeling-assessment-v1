...
### 4.4 i18n 真正落地 — ✅ 部分完成 (commit dc89ca5, 2026-05-02)
- `LOCALES.en` 字典已補（hero / nav / phase / action / result / quiz / theme / error / ui）
- header 已注入 `.lang-toggle` 按鈕（中/EN）
- 已標 `data-i18n` 的頁面：index.html（hero h1 + 主 CTA + nav buttons）、404.html（標題+三 CTA+尾註）
- **後續可逐步擴張**：assessment.html / dashboard.html / 4 個科普頁的標題與按鈕仍是 hardcoded 中文，切到 EN 不會翻譯。優先序低，等真有英文需求再標
